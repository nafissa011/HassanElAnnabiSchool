...
$stmt = $conn->prepare("SELECT Nom, Date_naiss, Email, Tél, Adresse, pfp, Password FROM admin WHERE Admin_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("UPDATE admin SET Nom = ?, Date_naiss = ?, Email = ?, Tél = ?, Adresse = ? WHERE Admin_id = ?");
    $stmt->bind_param("sssssi", $name, $date, $email, $tel, $adresse, $adminId);
    $stmt->execute();
...
if (isset($_FILES['profilePic']) && $_FILES['profilePic']['error'] === UPLOAD_ERR_OK) {
  ...
  if (in_array($fileExtension, $allowedExtensions)) {
      ...
      $newFileName = uniqid('admin_pfp_', true) . '.' . $fileExtension;
      $targetPath = $targetDirectory . $newFileName;

      if (move_uploaded_file($_FILES['profilePic']['tmp_name'], $targetPath)) {
          $stmt = $conn->prepare("UPDATE admin SET pfp = ? WHERE Admin_id = ?");
          $stmt->bind_param("si", $targetPath, $adminId);
          $stmt->execute();
...
<form action="profile.php" method="post" enctype="multipart/form-data">
    ...
    <input type="text" name="name" value="<?php echo htmlspecialchars($admin['Nom']); ?>" required>
    <input type="date" name="date" value="<?php echo htmlspecialchars($admin['Date_naiss']); ?>" required>
    <input type="email" name="email" value="<?php echo htmlspecialchars($admin['Email']); ?>" required>
    <input type="text" name="tel" value="<?php echo htmlspecialchars($admin['Tél']); ?>" required>
    <input type="text" name="adresse" value="<?php echo htmlspecialchars($admin['Adresse']); ?>" required>
