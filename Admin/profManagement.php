...
if (isset($_GET['delete'])) {
    ...
    $pdo->prepare('DELETE FROM professeur WHERE Prof_id = ?')->execute([$delete_id]);
    header("Location: profManagement.php");
    exit;
}

$stmt = $pdo->query('SELECT Prof_id, Nom, Email FROM professeur');
...
<h1 class="heading">Gestion des professeurs</h1>
...
<?php 
        ...
        foreach ($eleves as $eleve): ?>
            <div class="eleve">
                ...
                <div class="actions">
                    <a href="editProf.php?id=<?= $eleve['Prof_id'] ?>" class="btn btn-modify">Modifier</a>
                    <a href="?delete=<?= $eleve['Prof_id'] ?>" class="btn btn-red" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet professeur ?');">Supprimer</a>
                </div>
            </div>
        <?php 
        $i++; 
...
<a href="addProf.php" class="btn btn-add">Ajouter un prof</a>
