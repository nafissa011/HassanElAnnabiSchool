...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("UPDATE professeur SET Nom = ?, Date_naiss = ?, Adresse = ?, Email = ?, Tél = ?, Spécialité = ? WHERE Prof_id = ?");
...
// Récupérer les données du professeur actuel
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $pdo->prepare("SELECT * FROM professeur WHERE Prof_id = ?");
    $stmt->execute([$id]);
   ...
   <nav class="navbar">
    ...
    <?php if ($eleve): ?>
        <form action="editProf.php" method="post">
            <input type="hidden" name="id" value="<?= $eleve['Prof_id'] ?>">
            <p>
            ...
            <p>
                ...
                <input type="text" name="niveau" id="niveau" required value="<?= htmlspecialchars($eleve['Spécialité']) ?>">
            </p>
          
            <p>
                <button type="submit">Modifier professeur</button>
            </p>
        </form>
    <?php else: ?>
        <p>professeur not found.</p>
    <?php endif; ?>
