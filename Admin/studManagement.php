...
if (isset($_GET['delete'])) {
    ...
    $pdo->prepare('DELETE FROM eleve WHERE Elève_id = ?')->execute([$delete_id]);
    header("Location: studManagement.php");
    exit;
}
 
// Récupération des élèves
$stmt = $pdo->query('SELECT Elève_id, Nom, Email FROM eleve');
...
<section>
    <div class="container">
        <h1 class="heading">Gestion des élèves</h1>
        ...
        foreach ($eleves as $eleve): ?>
            <div class="eleve">
                <p><?= $i ?></p> 
                <p><?= htmlspecialchars($eleve['Nom']) ?></p> 
                <p><?= htmlspecialchars($eleve['Email']) ?></p> 
                <div class="actions">
                    <a href="editStud.php?id=<?= $eleve['Elève_id'] ?>" class="btn btn-modify">Modifier</a>
                    <a href="?delete=<?= $eleve['Elève_id'] ?>" class="btn btn-red" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet élève ?');">Supprimer</a>
                </div>
            </div>
        <?php 
        $i++; 
        ...
        <a href="addStud.php" class="btn btn-add">Ajouter un élève</a>
        <a href="addcoursStud.php" class="btn btn-add">Ajouter élèves/cours</a>
        <a href="managecoursStud.php" class="btn btn-add">Gérer élèves/cours</a>
