...
$stmt = $conn->prepare("SELECT Nom, Date_naiss, Email, Tél, Spécialité, Adresse, pfp, Password FROM professeur WHERE Prof_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    if (!empty($oldPass) && !empty($newPass) && !empty($confirmPass)) {
        if ($oldPass === $prof['Password']) {
            if ($newPass === $confirmPass) {
                $stmt = $conn->prepare("UPDATE professeur SET Password = ? WHERE Prof_id = ?");
                $stmt->bind_param("si", $newPass, $profId);
                $stmt->execute();
            } else {
    ...
    $stmt = $conn->prepare("UPDATE professeur SET Nom = ?, Date_naiss = ?, Email = ?, Tél = ?, Adresse = ?, Spécialité = ? WHERE Prof_id = ?");
    $stmt->bind_param("sssssi", $name, $date, $email, $tel, $adresse, $specialite, $profId);
    $stmt->execute();
...
if (isset($_FILES['profilePic']) && $_FILES['profilePic']['error'] === UPLOAD_ERR_OK) {
  ...
  if (in_array($fileExtension, $allowedExtensions)) {
      ...
      $newFileName = uniqid('prof_pfp_', true) . '.' . $fileExtension;
      $targetPath = $targetDirectory . $newFileName;

      if (move_uploaded_file($_FILES['profilePic']['tmp_name'], $targetPath)) {
          $stmt = $conn->prepare("UPDATE professeur SET pfp = ? WHERE Elève_id = ?");
          $stmt->bind_param("si", $targetPath, $profId);
          $stmt->execute();
...
<form action="profile.php" method="post" enctype="multipart/form-data">
    ...
    <input type="text" name="name" value="<?php echo htmlspecialchars($prof['Nom']); ?>" required>
    <input type="date" name="date" value="<?php echo htmlspecialchars($prof['Date_naiss']); ?>" required>
    <input type="email" name="email" value="<?php echo htmlspecialchars($prof['Email']); ?>" required>
    <input type="text" name="tel" value="<?php echo htmlspecialchars($prof['Tél']); ?>" required>
    <input type="text" name="adresse" value="<?php echo htmlspecialchars($prof['Adresse']); ?>" required>
    <input type="text" name="specialite" value="<?php echo htmlspecialchars($prof['Spécialité']); ?>" required>
